...
auth.onAuthStateChanged(async user => {
    if (!user) {
        // On sign-out, remove the localStorage session key so the next
        // login to this account on this browser gets a fresh session record.
        if (window._sessionUid) {
            localStorage.removeItem('c_sid_' + window._sessionUid);
            window._sessionId  = null;
            window._sessionUid = null;
        }
    ...
    // ── Session management ──────────────────────────────────────────────
    // localStorage persists across refreshes AND new tabs on the same browser,
    // so one browser = one session record. sessionStorage was the bug — it
    // gets cleared when the tab closes, so every new tab wrote a new record.
    const _sidKey = 'c_sid_' + user.uid;
    let _sid = localStorage.getItem(_sidKey);
    if (!_sid) {
        // First time this browser has logged in — create a new ID
        _sid = 's_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7);
        localStorage.setItem(_sidKey, _sid);
    }
    window._sessionId  = _sid;
    window._sessionUid = user.uid;

    // Detect browser + OS for the sessions panel display
    const _ua = navigator.userAgent;
    const _br = _ua.includes('Firefox') ? 'Firefox'
              : _ua.includes('Edg')     ? 'Edge'
              : _ua.includes('Chrome')  ? 'Chrome'
              : _ua.includes('Safari')  ? 'Safari' : 'Browser';
    const _os = _ua.includes('Android')                       ? 'Android'
              : _ua.includes('iPhone')||_ua.includes('iPad') ? 'iOS'
              : _ua.includes('Windows')                       ? 'Windows'
              : _ua.includes('Mac')                           ? 'macOS'
              : _ua.includes('Linux')                         ? 'Linux' : 'Unknown';

    // set() with {merge:true} means a page refresh just updates lastSeen
    // on the existing doc instead of creating a new one.
    db.collection('users').doc(user.uid)
      .collection('sessions').doc(_sid)
      .set({ browser: _br, os: _os,
             loginAt:  firebase.firestore.FieldValue.serverTimestamp(),
             lastSeen: firebase.firestore.FieldValue.serverTimestamp(),
             revoked:  false }, { merge: true })
      .catch(() => {});

    // Heartbeat: keep lastSeen fresh every 3 minutes
    setInterval(() => {
        db.collection('users').doc(user.uid)
          .collection('sessions').doc(_sid)
    ...
    // Watch for remote revocation — auto sign-out if another device revokes us
    db.collection('users').doc(user.uid)
      .collection('sessions').doc(_sid)
      .onSnapshot(snap => {
          if (snap.exists && snap.data().revoked === true) {
              localStorage.removeItem(_sidKey); // force new ID next login
...
async function openSessionsManagerInline() {
    ...
    try {
        ...
        list.innerHTML = '';
        if (snap.empty) {
            list.innerHTML = '<div class="empty-tab">No sessions recorded yet.</div>';
            return;
        }
        ...
        snap.forEach(doc => {
            ...
            ico.textContent = OSE[s.os] || '💻';
            ico.style.cssText = 'font-size:1.3rem;flex-shrink:0;';
            const info = document.createElement('div');
            info.style.cssText = 'flex:1;min-width:0;';
            const top = document.createElement('div');
            top.style.cssText = 'font-size:.85rem;font-weight:600;';
            ...
            sub.style.cssText = 'font-size:.7rem;color:var(--muted);';
            ...
            if (!isCur) {
                ...
                btn.onclick = async () => {
                    ...
                    try {
                        await db.collection('users').doc(me.id)
...
window.openNotifications = async () => {
    ...
    try {
        ...
        _renderNotifList(window._notifCache);
    } catch(e) {
...
function _renderNotifList(items) {
    ...
    list.innerHTML = '';
    if (!items.length) {
        list.innerHTML = '<div class="empty-tab" style="padding:28px 0;">No notifications yet. @mention someone to get started.</div>';
        return;
    }
    items.forEach(n => {
        const row = document.createElement('div');
        row.style.cssText = 'padding:12px 4px;border-bottom:1px solid rgba(255,255,255,.06);'
            + 'cursor:pointer;border-radius:6px;transition:background .12s;'
            + (n.read ? '' : 'background:rgba(56,189,248,.05);');
...
window.filterNotifications = function() {
    const q = (document.getElementById('notif-search') || {}).value || '';
    const lq = q.toLowerCase().trim();
    if (!window._notifCache) return;
    const filtered = lq
        ? window._notifCache.filter(n =>
            (n.fromName || '').toLowerCase().includes(lq) ||
            (n.preview  || '').toLowerCase().includes(lq))
        : window._notifCache;
    _renderNotifList(filtered);
};
